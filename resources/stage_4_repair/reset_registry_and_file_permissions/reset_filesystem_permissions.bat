...
::                1.0.3-TRON * Add proper version-specific commands to secedit database repair. Was incorrectly attempting to run Windows XP command on Windows 7 and up
...
subinacl.exe /noverbose /nostatistic /subdirectories %WinDir% /grant=administrators=f >%TEMP%\test.log
subinacl.exe /noverbose /nostatistic /subdirectories %WinDir% /grant=system=f >NUL

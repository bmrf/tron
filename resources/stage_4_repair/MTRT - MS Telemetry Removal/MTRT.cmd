...
REM Windows upgrade temp folders, will be cleaned and locked down
REM DEFAULT: "WINTEMP=$Windows.~BT,$Windows.~WS,Windows.old"
...
REM DEFAULT: "SCRIPTS=data"
SET "SCRIPTS=data"

REM Pull log path from command switch if invoked from TRON (Ex: MTRT.cmd "C:\Logs\tron\RAW_LOGS")
REM You can edit this manually, but it will will no longer save in the RAW_LOGS folder that Tron defines
REM DEFAULT: "MTRT_LOGPATH=%~1"
set "MTRT_LOGPATH=%~1"
...
REM Start Window 10 Only
IF /I NOT "%WIN_VER:~0,9%"=="Windows 1" (GOTO:SKIP_10_TWEAKS)
REM Disabled for Tron; it seems regular Flash also stores files here
REM CALL :LOGTXT "   Kill Adobe Flash that ships with Windows 10"
...
CALL :LOGCMD TASKKILL /F /IM  Explorer.exe /T
...
:: Do not want to disable this by default - this has legitimate uses, especially on mobile devices.
:: CALL :LOGTXT "   Disable geolocation Service"
:: CALL :LOGCMD SC STOP lfsvc 
:: CALL :LOGCMD SC CONFIG lfsvc START= disabled
...
rem CALL :LOGCMD ECHO. >>%WINDIR%\System32\Drivers\etc\hosts
CD /D "%DATA%"
...
:COMPLETE
CALL :LOGTXT "   All done! Suggest to restart your PC now."
ECHO   
timeout 6 >NUL
GOTO:EOF
:FAIL
